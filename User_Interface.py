...
class Interface(Frame):
    ...
    def __init__(self, fenetre, **kwargs):
        ...
        self.message2 = Label(self,justify=CENTER, text="")
        self.message2.grid(row=0,columnspan=4,column=0,pady=10)
        self.message = Label(self,justify=LEFT, text="Choose a STL file",anchor=W)
        ...
        self.message3=Label(self,text="Choose the Overhang Angle (must be between 40 to 50)",anchor=W)
        self.message3.grid(column=0,row=2,pady=5)
        ...
        self.support = Label(self,justify=CENTER, text="Choose your support shape")
        self.support.grid(row=4,column=0,rowspan=2,pady=5)
    ...
    def View(self):
        if self.bouton_OK["fg"] == 'green':
            ...
            ax2.set_ylabel('Y')
            ax2.set_zlabel('Z')
            ax2.set_xlim(np.min(my_mesh.min_),np.max(my_mesh.max_))
            ax2.set_ylim(np.min(my_mesh.min_),np.max(my_mesh.max_))
            ax2.set_zlim(0,np.max(my_mesh.max_))
